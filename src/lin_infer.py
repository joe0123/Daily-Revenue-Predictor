...
adr_model = Pipeline([("feature_selection", SelectFromModel(Lasso(alpha=1e-3, max_iter=1e+8))), \
...
# Initialization
    ...
    adr_x, adr_y, test_adr_x, _ = dataset.get_adr_data(onehot_x=True, scale=True)
    print(adr_x.shape)
    cancel_x, cancel_y, test_cancel_x, _ = dataset.get_cancel_data(onehot_x=True, scale=True)
